  ...
  class GetDomainConfig200Response
    ...
    def self.openapi_types
      {
        :'domain_name' => :'String',
        :'domain_id' => :'String',
        :'config' => :'GetDomainConfig200ResponseConfig'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Daily::GetDomainConfig200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Daily::GetDomainConfig200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@domain_name.nil? && @domain_name.to_s.length < 1
        invalid_properties.push('invalid value for "domain_name", the character length must be great than or equal to 1.')
      end

      if !@domain_id.nil? && @domain_id.to_s.length < 1
        invalid_properties.push('invalid value for "domain_id", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@domain_name.nil? && @domain_name.to_s.length < 1
      return false if !@domain_id.nil? && @domain_id.to_s.length < 1
      true
    end
    ...
    def domain_id=(domain_id)
      ...
      if domain_id.to_s.length < 1
        fail ArgumentError, 'invalid value for "domain_id", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [domain_name, domain_id, config].hash
    end
