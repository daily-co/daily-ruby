...
module Daily
  class DailyLiveStreamingOptions
    ...
    def self.openapi_types
      {
        ...
        :'rtmp_url' => :'DailyLiveStreamingOptionsRtmpUrl',
        :'endpoints' => :'Array<DailyStreamingEndpoint>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Daily::DailyLiveStreamingOptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Daily::DailyLiveStreamingOptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'rtmp_url')
        self.rtmp_url = attributes[:'rtmp_url']
      end
    ...
    def hash
      [width, height, fps, video_bitrate, audio_bitrate, min_idle_time_out, max_duration, background_color, instance_id, layout, rtmp_url, endpoints].hash
    end
